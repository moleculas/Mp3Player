...
import { deleteSongById, getAllSongsDashboard } from "../api";
...
const DashboardSongs = () => {
  ...
  useEffect(() => {
    if (!allSongs) {
      getAllSongsDashboard().then((data) => {
        dispatch({
  ...
  useEffect(() => {
    if (songFilter.length > 0) {
      const miBusqueda = songFilter.toLowerCase();
...
export const SongContainer = ({ data }) => {
  return (
    <div className=" w-full  flex flex-wrap gap-3  items-center justify-evenly">
      {data &&
        data.map((song, i) => (          
          i < 8 && (<SongCard key={song._id} data={song} index={i} />)
        ))}
    </div>
  );
};
