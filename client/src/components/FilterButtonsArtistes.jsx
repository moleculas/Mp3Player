...
const FilterButtonsArtistes = ({ filterData, flag }) => {
  ...
  useEffect(() => {
    if (filterMenu) {
      dispatch({ type: actionType.SET_FILTER_MENU_ARTISTES, filterMenuArtistes: true });
  ...
  useEffect(() => {
    if (filterMenuAlbums || filterMenuCategories) {
      setFilterMenu(false);
    };
  }, [filterMenuAlbums, filterMenuCategories]);
    ...
    <div className="w-full sm:w-auto border border-gray-300 rounded-md px-4 py-1 relative cursor-pointer hover:border-gray-400">
      ...
      {filterData && filterMenu && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 50 }}
          className="w-full sm:w-96 z-50 backdrop-blur-xl max-h-[300px] mt-2 overflow-y-scroll scrollbar-thin scrollbar-track-gray-200 scrollbar-thumb-gray-400 py-2 flex flex-col rounded-md shadow-md absolute top-8 left-0"
        >
          {filterData?.map((data) => (
            <div
              key={data.name}
...
export default FilterButtonsArtistes;
