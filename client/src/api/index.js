...
export const getAllSongsDashboard = async () => {
  try {
    const res = await axios.get(`${baseURL}api/songs/getAllDashboard`);
    return res.data;
  } catch (error) {
    return null;
  }
...
export const uploadFile = async (file) => {
  try {
    ...
    const res = axios.post(`${baseURL}api/songs/uploadFile`, formData);
    return res;
  } catch (error) {
    return null;
  }
